...
import { MessageSquare, Send, RefreshCw, Briefcase, User, Clock } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 text-left animate-in fade-in duration-300 relative h-[calc(100vh-140px)] flex flex-col">
      ...
      <div className="flex justify-between items-center shrink-0">
        <div>
          <h2 className="text-2xl font-display font-extrabold text-white">Inbox & Messages 💬</h2>
          <p className="text-gray-400 text-sm mt-1">Communicate directly with corporate recruiters and hiring managers.</p>
        </div>
        <button 
          onClick={() => { fetchConversations(); if (activeSession) fetchMessages(activeSession.id); }}
          className="p-2.5 rounded-xl bg-dark-card border border-dark-border hover:border-brand-500/30 text-gray-450 hover:text-white transition-all flex items-center justify-center"
        >
          <RefreshCw size={15} />
        </button>
      </div>
        ...
        <div className="w-full md:w-80 shrink-0 flex flex-col glass border border-dark-border/60 rounded-2xl overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-dark-border/40 p-2 space-y-1">
            {conversations.length === 0 ? (
              <div className="py-12 text-center text-dark-muted text-xs flex flex-col items-center justify-center space-y-3">
                <MessageSquare size={24} className="text-dark-muted/60" />
                <p>No active conversations yet.</p>
                <p className="text-[10px] max-w-[200px] leading-relaxed mx-auto text-dark-muted/80">
                  When a recruiter contacts you, your messages will appear here.
                </p>
              </div>
            ) : (
              conversations.map((c) => {
                  ...
                  >
                    <div className="w-9 h-9 rounded-xl bg-brand-500/10 border border-brand-500/20 flex items-center justify-center text-brand-400 font-bold shrink-0">
                      {c.otherParticipant.avatarUrl ? (
                        <img src={c.otherParticipant.avatarUrl} alt="Recruiter Avatar" className="w-full h-full rounded-xl object-cover" />
                      ) : (
                        <User size={15} />
                      )}
                    </div>
                    <div className="flex-1 min-w-0 space-y-0.5">
                      ...
                      <p className="text-[10px] text-emerald-400 font-semibold truncate">
                        {c.otherParticipant.companyName || 'Corporate Recruiter'}
                      </p>
                      {c.latestMessage ? (
        ...
        <div className="flex-1 min-w-0 glass border border-dark-border/60 rounded-2xl flex flex-col overflow-hidden">
          {activeSession ? (
              ...
              <div className="p-4 border-b border-dark-border/60 bg-dark-bg/40 flex justify-between items-center shrink-0">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-emerald-400 font-semibold mt-0.5">
                      {activeSession.otherParticipant.companyName} • {activeSession.otherParticipant.designation || 'Hiring Manager'}
                    </p>
                  </div>
                </div>
                {activeSession.jobTitle && (
                  <div className="hidden sm:flex items-center space-x-1.5 bg-dark-card border border-dark-border/80 px-2.5 py-1 rounded-lg text-[9px] text-gray-300">
                    ...
                    <span>Role: <strong className="text-white">{activeSession.jobTitle}</strong></span>
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-dark-deep/10">
                {messagesLoading && messages.length === 0 ? (
                  ...
                  <div className="h-full flex flex-col justify-center items-center text-center text-dark-muted text-xs space-y-2">
                    ...
                    <p className="text-[10px]">Type a greeting below to initiate info exchange.</p>
                  </div>
                ) : (
                  messages.map((m) => {
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-center text-dark-muted text-xs p-6 space-y-3">
              ...
              <p className="font-bold text-white">No Conversation Active</p>
              <p className="max-w-xs leading-relaxed text-[11px]">
                Select an inbox conversation from the active chats menu on the left to review messages and coordinate hiring details.
              </p>
            </div>
          )}
        </div>
